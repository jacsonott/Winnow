...
change that lands somewhere new comes back to the same row, a row the
narrower view no longer has clears the cursor rather than leaving a stale
number behind, and the header box's kept offset is untouched.
"""
...
def test_the_header_box_keeps_its_offset_while_the_cursor_follows(page):
    ...
    try:
        ...
        # The viewport did not move, and the cursor is on the same row at
        # its new position (three of every four rows above it survive).
