...
- Derived datetime columns aren't available on merged tables, and their values
  aren't covered by substring search (the source column they're computed from
  is, so the row is still findable).
